...
const InstallationTutorial = ({bottomRef}) => {
  ...
  if (loading) {
    return (
      <section id="tutorial" className="py-12 md:py-20 bg-foreground text-primary-foreground">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="mt-4 text-primary-foreground/60 text-sm md:text-base">Carregando tutorial...</p>
        </div>
      </section>
    );
  }
  ...
  if (totalSteps === 0) {
    return (
      <section id="tutorial" className="py-12 md:py-20 bg-foreground text-primary-foreground">
        <div className="container mx-auto px-4 text-center">
          <ImageIcon className="w-12 h-12 md:w-16 md:h-16 text-primary-foreground/30 mx-auto mb-4" />
          <p className="text-primary-foreground/60 text-sm md:text-base">Tutorial em breve...</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section id="tutorial" className="py-12 md:py-20 bg-foreground text-primary-foreground" ref={bottomRef}>
      <div className="container mx-auto px-4">
        {/* Header */}
        <div className="text-center mb-6 md:mb-12">
          <div className="inline-flex items-center gap-1.5 md:gap-2 bg-primary/20 px-3 py-1.5 md:px-4 md:py-2 rounded-full mb-3 md:mb-4">
            <Download className="w-3 h-3 md:w-4 md:h-4 text-primary" />
            <span className="text-xs md:text-sm font-semibold text-primary">PASSO A PASSO</span>
          </div>
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-2 md:mb-4">
            {sectionTitle[language]}
          </h2>
          <p className="text-primary-foreground/70 text-sm md:text-base lg:text-lg max-w-2xl mx-auto px-2">
        ...
        <div className="hidden md:block max-w-3xl mx-auto mb-8">
            ...
            {Array.from({ length: totalSteps }).map((_, index) => (
              <button
                ...
                className={`flex items-center justify-center w-9 h-9 lg:w-11 lg:h-11 rounded-full font-bold text-xs lg:text-sm transition-all duration-300 border-2 ${
                  index === currentStep
                    ? "bg-gradient-to-r from-primary to-pink-500 text-white scale-125 shadow-glow border-transparent"
                    : index < currentStep
                    ? "bg-green-500 text-white border-green-400 shadow-md"
                    : "bg-muted/20 text-muted-foreground border-muted/30 hover:bg-muted/40 hover:border-primary/50"
                }`}
              >
                {index < currentStep ? (
          ...
          <div className="relative h-3 bg-muted/20 rounded-full overflow-hidden border border-muted/30">
            <motion.div
              className="absolute top-0 left-0 h-full bg-gradient-to-r from-primary via-pink-500 to-primary rounded-full shadow-glow"
              initial={{ width: 0 }}
          ...
          <div className="text-center mt-3">
            ...
            <span className="text-primary-foreground/50"> de </span>
            <span className="text-primary-foreground/70 font-medium">{totalSteps}</span>
          </div>
        </div>

        {/* Tutorial Content */}
        <div className="max-w-4xl mx-auto">
          <div className="bg-card/5 backdrop-blur-sm rounded-2xl md:rounded-3xl p-4 md:p-6 lg:p-8 border border-primary/10">
              ...
              >
                ...
                <div className="relative flex-shrink-0">
                  <div className="relative w-[200px] sm:w-[240px] md:w-[280px] lg:w-[320px] mx-auto">
                    {/* Phone Frame */}
                    <div className="relative bg-gradient-to-b from-gray-800 to-gray-900 rounded-[2rem] md:rounded-[3rem] p-2 md:p-3 shadow-2xl">
                      {/* Screen */}
                      <div className="relative rounded-[1.75rem] md:rounded-[2.5rem] overflow-hidden bg-black">
                        <img
                    ...
                    {/* Step Badge */}
                    <div className="absolute -top-2 -right-2 md:-top-4 md:-right-4 w-10 h-10 md:w-14 md:h-14 rounded-full bg-gradient-to-r from-primary to-pink-500 flex items-center justify-center shadow-lg">
                      <span className="text-white font-bold text-sm md:text-lg">{currentStep + 1}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 text-center lg:text-left">
                  <h3 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold mb-2 md:mb-4 text-primary-foreground">
                    {currentStepData.title}
                  </h3>
                  <p className="text-primary-foreground/70 text-sm md:text-base lg:text-lg leading-relaxed mb-4 md:mb-6">
                  ...
                  <div className="flex items-center justify-center lg:justify-start gap-3 md:gap-4">
                    ...
                    {currentStep < totalSteps - 1 ? (
                      ...
                      <Button
                        size="lg"
                        className="flex items-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 shadow-lg shadow-green-500/30 hover:scale-105 active:scale-95 text-sm md:text-base px-6 md:px-8 py-2 md:py-3 font-bold transition-all duration-300"
                      >
          ...
          {Array.from({ length: totalSteps }).map((_, index) => (
            <button
              ...
              className={`rounded-full transition-all duration-300 ${
                index === currentStep
                  ? "bg-gradient-to-r from-primary to-pink-500 w-8 md:w-10 h-3 md:h-4 shadow-glow"
                  : index < currentStep
                  ? "bg-green-500 w-3 h-3 md:w-4 md:h-4 shadow-md"
                  : "bg-white/20 w-2.5 h-2.5 md:w-3 md:h-3 hover:bg-white/40"
              }`}
        ...
        <div className="text-center mt-4 lg:hidden">
          ...
          <span className="text-primary-foreground/50 text-sm md:text-base"> / {totalSteps}</span>
        </div>
      </div>
    </section>
  );
